  ...
  const rows =
    state?.history.filter(
      (h) =>
        ...
        h.kind === filter ||
        (filter === "upgrade" && h.kind === "nugget-upgrade") ||
        (filter === "battle" && h.kind.startsWith("battle-")) ||
        (filter === "sell" && h.kind === "sell-batch"),
    ) ?? [];
  return (
          ...
          <tbody>
            {rows.map((h) => (
              <tr key={h.id}>
                ...
                <td>
                  {h.kind === "sell-batch"
                    ? `Sold ${h.detail.sold} auto-roll cards`
                    : h.kind === "trade"
                      ? `Trade with ${h.detail.with}`
                      : h.kind === "transfer"
                        ? `Nugget transfer`
                        : h.kind === "battle-entry"
                          ? `Battle entry · ${h.detail.mode}`
                          : h.kind === "battle-refund"
                            ? "Battle declined · Entry refunded"
                            : h.kind === "battle-result"
                              ? h.detail.tie
                                ? "Tie · Own cards kept"
                                : h.detail.winnerId === state?.player.id
                                  ? "Won · All cards received"
                                  : "Lost · Opponent received the cards"
                              : h.kind === "case" || h.kind === "battle-case"
                                ? (state?.catalog.items.find(
                                    (i) => i.id === h.detail.itemId,
                                  )?.name ?? "Item collected")
                                : h.kind === "upgrade" && h.detail.effect
                                  ? h.detail.won
                                    ? String(h.detail.effect) +
                                      " · " +
                                      String(h.detail.value) +
                                      " SN card value"
                                    : "Card lost · " +
                                      String(h.detail.effect) +
                                      " attempt"
