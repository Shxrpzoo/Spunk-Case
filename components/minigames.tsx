...
export function Minigame({
  ...
  const odds = cardMode
    ...
    valid =
      Number.isInteger(n) &&
      n >= 1 &&
      n <= (kind === "coin" ? 9000000000000 : 1000000) &&
      (kind !== "coin" || n + (state?.player.balance ?? 0) <= 9000000000000) &&
  ...
  async function spin() {
    ...
    if (!cardMode && !valid) {
      setError(
        kind === "coin"
          ? "Enter a whole amount within your balance and account balance limit."
        ...
        >
          ...
          {kind === "upgrade" ? (
            ...
            <div className="coin-stage">
              ...
              <div
                className={
                  "coin-3d " + (blackout && spinning ? "blackout-coin" : "")
                }
                style={{
              ...
              >
                <div className="coin-face heads">
                  <span>
                    {blackout && spinning ? "BLACKOUT" : "SPUNK NUGGET"}
                  </span>
                  <strong>{blackout && spinning ? "?" : "S"}</strong>
                  <small>
                    {blackout && spinning ? "REVEAL AT LANDING" : "HEADS"}
                  </small>
                </div>
                <div className="coin-face tails">
                  <span>
                    {blackout && spinning ? "BLACKOUT" : "THE OTHER SIDE"}
                  </span>
                  <strong>{blackout && spinning ? "?" : "✦"}</strong>
                  <small>
                    {blackout && spinning ? "REVEAL AT LANDING" : "TAILS"}
                  </small>
                </div>
              </div>
            </div>
        ...
        <div className="game-console">
          ...
          {cardMode ? (
            ...
            <>
              ...
              {kind === "coin" && (
                <label className="blackout-toggle">
                  <input
                    type="checkbox"
                    checked={blackout}
                    disabled={busy || spinning}
                  ...
                  <span>
                    Blackout mode{" "}
                    <small>Both sides stay hidden until the coin lands.</small>
                  </span>
                </label>
              )}
              <label className="field">
                  ...
                  max={
                    kind === "coin"
                      ? Math.min(
                          ...
                          9000000000000 - (state?.player.balance ?? 0),
                        )
                      : 1000000
                  }
                  step="1"
                ...
                <button
                  ...
                  onClick={() =>
                    setWager(
                      String(
                        Math.min(
                          kind === "coin" ? 9000000000000 : 1000000,
                          Math.max(
                            1,
