...
import { battleAction, battleInbox, battleDetail } from "@/server/battles";
import { autoBatch } from "@/server/auto-roll";
...
async function handler(req: NextRequest, ctx: Context) {
  ...
  if (path === "social" && isPost) {
    await rateLimit(db, "social:" + id, 100, 60);
    return ok(await socialAction(db, id, body));
  }
  if (path === "battle" && isPost) {
    await rateLimit(db, "battle:" + id, 100, 60);
    return ok(await battleAction(db, id, body));
  }
  if (path === "battle" && !isPost)
    return ok(
