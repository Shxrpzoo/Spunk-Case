    ...
    items: z
      .array(
        z.object({
          ...
          value: z.number().int().min(1).max(100000000).optional(),
...
export async function readCatalog(db: DB): Promise<Catalog> {
 ...
 'cases',(SELECT coalesce(jsonb_agg(to_jsonb(ca)||jsonb_build_object('weights',(SELECT coalesce(jsonb_agg(jsonb_build_object('itemId',w.item_id,'weight',w.weight) ORDER BY w.item_id),'[]') FROM spunk.case_items w WHERE w.case_id=ca.id)) ORDER BY COALESCE((to_jsonb(ca)->>'sort_order')::int,CASE ca.id WHEN 'basic' THEN 1 WHEN 'ash' THEN 2 WHEN 'satchel' THEN 3 WHEN 'goon' THEN 4 ELSE 5 END),ca.id),'[]') FROM spunk.cases ca),
...
export async function writeCatalog(db: DB, input: unknown) {
  ...
  for (const i of c.items)
    await db.query(
      "INSERT INTO spunk.items(id,name,rarity,image,value,mystery) VALUES($1,$2,$3,$4,$5,$6) ON CONFLICT(id) DO UPDATE SET name=$2,rarity=$3,image=$4,value=COALESCE($5,spunk.items.value),mystery=$6",
      [i.id, i.name, i.rarity, i.image, i.value ?? null, i.mystery ?? false],
    );
  for (const ca of c.cases) {
    await db.query(
  ...
  await db.query(
    "INSERT INTO spunk.settings(id,value) VALUES(1,$1::text::jsonb) ON CONFLICT(id) DO UPDATE SET value=$1::text::jsonb,version=spunk.settings.version+1",
