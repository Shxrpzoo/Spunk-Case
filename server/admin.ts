...
import { awardCompletions, changeBalance, getState } from "./game";
    ...
    if (a.action === "catalog") {
      ...
      if (a.action === "give-item" || a.action === "remove-item") {
        ...
        if (a.action === "give-item") {
          ...
          await tx.query(
            "DELETE FROM spunk.player_items WHERE player_id=$1 AND item_id=$2 AND quantity=1",
            [p.id, a.itemId],
          );
          await tx.query(
            "UPDATE spunk.player_items SET quantity=quantity-1 WHERE player_id=$1 AND item_id=$2",
            [p.id, a.itemId],
          );
        }
      }
