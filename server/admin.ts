  ...
  return db.transaction(async (tx) => {
    ...
    if (a.action === "catalog") {
      ...
      if (a.action === "reset-player" || a.action === "reset-collection") {
        ...
        const [autoTable] = await tx.query<{ name: string | null }>(
          "SELECT to_regclass('spunk.auto_receipts')::text name",
        );
        if (autoTable.name)
          await tx.query(
            "UPDATE spunk.auto_receipts SET consumed_at=now() WHERE player_id=$1 AND consumed_at IS NULL",
            [p.id],
          );
      }
      ...
      await tx.query(
        "INSERT INTO spunk.history(id,player_id,kind,amount,detail) VALUES($1,$2,'admin',$3,$4::text::jsonb)",
        [
          ref,
          p.id,
          Number(
    ...
    await tx.query(
      "INSERT INTO spunk.admin_audit(id,action,detail) VALUES($1,$2,$3::text::jsonb)",
      [randomUUID(), a.action, JSON.stringify(safe)],
    );
    const response = { ok: true };
    await tx.query(
      "INSERT INTO spunk.admin_requests VALUES($1,$2,$3::text::jsonb)",
      [a.key, fingerprint, JSON.stringify(response)],
    );
    return response;
  });
}
