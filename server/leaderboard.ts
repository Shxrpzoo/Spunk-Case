...
export async function leaderboard(db: DB): Promise<Leaderboards> {
 ...
 valued AS (SELECT c.*,COALESCE((to_jsonb(i)->>'value')::bigint,($1::text::jsonb->>i.rarity)::int) * ($2::text::jsonb->>c.effect)::int value FROM cards c JOIN spunk.items i ON i.id=c.item_id),
