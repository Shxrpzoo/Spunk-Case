...
import { ensureBatch, consumeAutoReceipt } from "./auto-roll";
...
export const actionSchema = z.discriminatedUnion("kind", [
  ...
  z.object({
    kind: z.literal("coin"),
    wager: z.number().int().min(1).max(9000000000000),
...
export async function awardCompletions(
  ...
  const groups = [
    ...rarityGroups(c).map((g) => ({
      ...
      reward: collectionReward(c, g.caseId, g.rarity),
...
export async function play(
  ...
  return db.transaction(async (tx) => {
    ...
    if (a.kind === "daily" || a.kind === "free") {
      ...
      if (a.batchId) await ensureBatch(tx, p.id, a.batchId);
      ...
      out.item = c.items.find((i) => i.id === out.itemId);
    ...
    } else if (a.kind === "sell-batch") {
      const receipts = await tx.query<{ opening_id: string; item_id: string }>(
        "SELECT r.opening_id,r.item_id FROM spunk.auto_receipts r JOIN spunk.items i ON i.id=r.item_id WHERE r.batch_id=$1 AND r.player_id=$2 AND r.consumed_at IS NULL AND ($3::text IS NULL OR i.rarity=$3) ORDER BY r.created_at,r.opening_id",
        [a.batchId, p.id, a.rarity ?? null],
      );
      if (!receipts.length)
        throw new GameError("No unsold cards remain in that batch selection.");
      out.payout = 0;
      out.sold = 0;
      out.batchId = a.batchId;
      for (const r of receipts) {
        const item = c.items.find((i) => i.id === r.item_id)!;
        await takeCard(tx, p.id, item.id, "RAW", r.opening_id);
        out.payout += cardValue(item, "RAW");
        out.sold++;
      }
      await changeBalance(tx, p, out.payout, "AUTO_BATCH_SALE", id);
      ...
      if (p.balance + a.wager > 9000000000000)
        throw new GameError(
          "This wager could exceed the account balance limit. Choose a smaller amount.",
        );
    ...
    if (a.kind === "case" && a.batchId) {
      out.batchId = a.batchId;
      await tx.query(
        "INSERT INTO spunk.auto_receipts(opening_id,batch_id,player_id,item_id) VALUES($1,$2,$3,$4)",
        [id, a.batchId, p.id, out.itemId],
      );
    }
...
export async function getState(db: DB, id: string): Promise<State> {
 ...
 'serverTime',clock_timestamp()::text) AS data`,
    ...
    const discovered = new Set(snapshot.data.discoveries);
    catalog.items = catalog.items.map((i) =>
      i.mystery && !discovered.has(i.id) ? hiddenMystery(i) : i,
    );
    return { player, ...snapshot.data, catalog };
  });
}
...
export async function takeCard(
  ...
  if (raw) await consumeAutoReceipt(db, playerId, itemId, receiptId);
}
